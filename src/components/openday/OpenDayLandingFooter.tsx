import { Link } from 'react-router-dom';
import { OPENDAY_CONFIG } from '../../config/openday';
import { STUDIO } from '../../config/constants';

export const OpenDayLandingFooter = () => {
  const { PHONE_NUMBER, PHONE_HREF } = OPENDAY_CONFIG;
  const formattedPhone = PHONE_NUMBER.replace(/(\d{3})(\d{3})(\d{4})/, '$1 $2 $3');

  return (
    <footer className="relative z-10 bg-primary text-white">
      <div className="mx-auto max-w-6xl px-6 py-12 md:py-16 lg:px-12">
        <div className="grid grid-cols-1 items-start gap-10 md:grid-cols-3 md:gap-12">
          <div>
            <div className="mb-3 text-2xl font-bold">
              Studio <span className="text-accent">Fisyo</span>
            </div>
            <p className="text-sm leading-relaxed text-white/60">
              Fisioterapia, Pilates Clinico e
              <br />
              Salute Integrata a Felino (PR)
            </p>
          </div>

          <div>
            <div className="mb-4 text-sm font-bold uppercase tracking-widest text-white/65">
              Vieni a trovarci
            </div>
            <p className="mb-3 text-sm leading-relaxed text-white/70">
              Via Aldo Moro 1/A
              <br />
              43035 Felino (PR)
            </p>
            <a
              href={`tel:${PHONE_HREF}`}
              className="mb-1 block text-sm font-bold text-accent transition-colors hover:text-accent/80"
            >
              {formattedPhone}
            </a>
            <a
              href={`mailto:${STUDIO.email}`}
              className="block text-sm text-white/60 transition-colors hover:text-white"
            >
              {STUDIO.email}
            </a>
          </div>

          <div>
            <div className="mb-4 text-sm font-bold uppercase tracking-widest text-white/65">
              Scopri di più
            </div>
            <Link
              to="/"
              className="mb-4 inline-flex items-center gap-2 text-sm font-bold text-accent transition-colors hover:text-white"
            >
              Visita www.studiofisyo.com
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />
              </svg>
            </Link>
            <div className="mt-4 flex gap-4">
              <a
                href={STUDIO.instagram}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Instagram"
                className="flex h-10 w-10 items-center justify-center rounded-full bg-white/10 transition-colors hover:bg-accent/20"
              >
                <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                </svg>
              </a>
              <a
                href={STUDIO.facebook}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Facebook"
                className="flex h-10 w-10 items-center justify-center rounded-full bg-white/10 transition-colors hover:bg-accent/20"
              >
                <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </a>
            </div>
          </div>
        </div>

        <div className="mt-10 flex flex-col items-center justify-between gap-3 border-t border-white/10 pt-6 text-xs text-white/65 md:flex-row">
          <span>2026 Studio Fisyo. P.IVA 02551160340</span>
          <div className="flex gap-4">
            <a
              href="https://www.iubenda.com/privacy-policy/17fdc897"
              target="_blank"
              rel="noopener noreferrer"
              className="transition-colors hover:text-white/60"
            >
              Informativa Privacy
            </a>
            <span>.</span>
            <a
              href="https://www.iubenda.com/privacy-policy/17fdc897/cookie-policy"
              target="_blank"
              rel="noopener noreferrer"
              className="transition-colors hover:text-white/60"
            >
              Gestione Cookie
            </a>
          </div>
        </div>
      </div>
    </footer>
  );
};
